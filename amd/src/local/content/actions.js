 ...
 * Actions
 *
 * @module     format_eloflexsections/local/content/actions
...
export default class extends BaseComponent {
    ...
    create() {
        ...
        this.name = 'content_actions_eloflexsections';
        // Default query selectors.
        this.selectors = {
            ACTIONLINK: `[data-action-eloflexsections]`,
    ...
    _dispatchClick(event) {
        ...
        const methodName = this._actionMethodName(target.getAttribute('data-action-eloflexsections'));
    ...
    _requestDelete(target, event) {
        ...
        if (cmList.length || sectionInfo.hassummary || sectionInfo.rawtitle || sectionInfo.children.length) {
            getStrings([
                ...
                {key: 'confirmdelete', component: 'format_eloflexsections'},
    ...
    _requestMergeup(target, event) {
        ...
        if (cmList.length || sectionInfo.hassummary || sectionInfo.rawtitle || sectionInfo.children.length) {
            getStrings([
                {key: 'confirm', component: 'moodle'},
                {key: 'confirmmerge', component: 'format_eloflexsections'},
                {key: 'yes', component: 'moodle'},
                {key: 'no', component: 'moodle'}
            ]).done((strings) => {
                Notification.confirm(
    ...
    _requestMoveSection(target, event) {
        ...
        ModalFactory.create({
            ...
            .then(modal => {
                Templates.render('format_eloflexsections/local/content/movesection', data).
                then((body) => {
                    modal.setBody(body);
    ...
    _requestMoveCm(target, event) {
        ...
        ModalFactory.create({
            ...
            .then(modal => {
                Templates.render('format_eloflexsections/local/content/movecm', data).
                then((body) => {
                    modal.setBody(body);
