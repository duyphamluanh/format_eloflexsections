...
namespace format_eloflexsections\output\courseformat\content\section;
    ...
    public function section_control_items() {
        ...
        if (has_capability('moodle/course:update', $coursecontext) && $section->section &&
            ...
            $controls['addsubsection'] = [
                ...
                'icon' => 't/add',
                'name' => get_string('addsubsection', 'format_eloflexsections'),
                'pixattr' => ['class' => ''],
                'attr' => [
                    'class' => 'editing_addsubsection',
                    'data-action-eloflexsections' => 'addSubSection',
        ...
        if ($section->section && has_capability('moodle/course:update', $coursecontext) &&
            ...
            $attrs = [
                'data-action-eloflexsections' => 'sectionSwitchCollapsed',
            ...
            if ($section->collapsed == FORMAT_ELOFLEXSECTIONS_COLLAPSED) {
                $controls['collapsed'] = [
                    'url' => $collapseurl,
                    'icon' => 't/expanded',
                    'name' => get_string('showexpanded', 'format_eloflexsections'),
                    'pixattr' => ['class' => ''],
                    'attr' => $attrs,
                ];
            } else {
                $controls['collapsed'] = [
                    'url' => $collapseurl,
                    'icon' => 't/collapsed',
                    'name' => get_string('showcollapsed', 'format_eloflexsections'),
                    'pixattr' => ['class' => ''],
                    'attr' => $attrs,
                ];
            }
        ...
        if ($section->parent && has_capability('moodle/course:update', $coursecontext) &&
            ...
            $controls['mergeup'] = [
                'url' => $mergeupurl,
                'icon' => 'mergeup',
                'iconcomponent' => 'format_eloflexsections',
                'name' => get_string('mergeup', 'format_eloflexsections'),
                'pixattr' => ['class' => ''],
                'attr' => [
                    'class' => 'editing_mergeup',
                    'data-action-eloflexsections' => 'mergeup',
                    'data-id' => $section->id,
                ],
            ];
        }
        ...
        if (has_capability('moodle/course:update', $coursecontext) && $section->section &&
            ...
            $controls['moveeloflexsections'] = [
                'url' => $moveurl,
                'icon' => 'i/dragdrop',
                'name' => get_string('move', 'moodle'),
                'pixattr' => ['class' => ''],
                'attr' => [
                    'data-action-eloflexsections' => 'moveSection',
                    'data-id' => $section->id,
        ...
        if ($section->section == $this->format->get_viewed_section()) {
            ...
            $parentcontrols['delete']['attr']['data-action-eloflexsections'] = 'delete';
        }
