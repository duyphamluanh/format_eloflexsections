...
define('FORMAT_ELOFLEXSECTIONS_COLLAPSED', 1);
define('FORMAT_ELOFLEXSECTIONS_EXPANDED', 0);
define('FORMAT_ELOFLEXSECTIONS_EXPANDED_ALWAYS_YES', 0);
define('FORMAT_ELOFLEXSECTIONS_EXPANDED_ALWAYS_NO', 1);
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_eloflexsections extends core_courseformat\base {
    ...
    public function uses_indentation(): bool {
        return (get_config('format_eloflexsections', 'indentation')) ? true : false;
    }
    ...
    public function get_max_section_depth(): int {
        $limit = (int)get_config('format_eloflexsections', 'maxsectiondepth');
    ...
    /**
     * Returns the default section name for the eloflexsections course format.
     *
    ...
    public function get_default_section_name($section) {
        if ($section->section == 0) {
            ...
            return get_string('section0name', 'format_eloflexsections');
        } else {
    ...
    public function get_view_url($section, $options = []) {
        ...
        if (array_key_exists('sr', $options)) {
            ...
            if ($section->collapsed == FORMAT_ELOFLEXSECTIONS_COLLAPSED) {
    ...
    public function section_format_options($foreditform = false): array {
        return array(
            ...
            'collapsed' => array(
                'type' => PARAM_INT,
                'label' => get_string('displaycontent', 'format_eloflexsections'),
                'element_type' => 'select',
                'element_attributes' => array(
                    array(
                        FORMAT_ELOFLEXSECTIONS_EXPANDED => new lang_string('showexpanded', 'format_eloflexsections'),
                        FORMAT_ELOFLEXSECTIONS_COLLAPSED => new lang_string('showcollapsed', 'format_eloflexsections'),
                    )
                ),
                'cache' => true,
                'cachedefault' => FORMAT_ELOFLEXSECTIONS_EXPANDED,
                'default' => COURSE_DISPLAY_SINGLEPAGE,
            ),
            'alwaysexpand' => array(
                'type' => PARAM_INT,
                'label' => get_string('alwaysexpand', 'format_eloflexsections'),
                'element_type' => 'select',
                'element_attributes' => array(
                    array(
                        FORMAT_ELOFLEXSECTIONS_EXPANDED_ALWAYS_YES => new lang_string('expandalwaysyes', 'format_eloflexsections'),
                        FORMAT_ELOFLEXSECTIONS_EXPANDED_ALWAYS_NO => new lang_string('expandalwaysno', 'format_eloflexsections'),
                    )
                ),
                'cache' => true,
                'default' => FORMAT_ELOFLEXSECTIONS_EXPANDED_ALWAYS_NO,
                'cachedefault' => 0,
            ),
        );
    }
     ...
     *
     * Eloflexsections format uses the following options:
     * - showsection0title
     *
     * @param bool $foreditform
    ...
    public function course_format_options($foreditform = false) {
        ...
        if ($courseformatoptions === false) {
            $courseformatoptions = [
                'showsection0title' => [
                    'default' => get_config('format_eloflexsections', 'showsection0titledefault') ?? 0,
                    'type' => PARAM_BOOL,
                ],
                'courseindexdisplay' => [
                    'default' => get_config('format_eloflexsections', 'courseindexdisplay') ?? 0,
                    'type' => PARAM_INT,
                ],
                'accordion' => [
                    'default' => (bool)get_config('format_eloflexsections', 'accordion'),
                    'type' => PARAM_BOOL,
                ],
                'cmbacklink' => [
                    'default' => (bool)get_config('format_eloflexsections', 'cmbacklink'),
                    'type' => PARAM_BOOL,
                ],
            ];
        }
        if ($foreditform && !isset($courseformatoptions['showsection0title']['label'])) {
            ...
            $courseformatoptionsedit = [
                'showsection0title' => [
                    'label' => new lang_string('showsection0title', 'format_eloflexsections'),
                    'help' => 'showsection0title',
                    'help_component' => 'format_eloflexsections',
                    'element_type' => 'advcheckbox',
                ],
                'courseindexdisplay' => [
                    'label' => new lang_string('courseindexdisplay', 'format_eloflexsections'),
                    'element_type' => 'select',
                    'element_attributes' => [$options],
                ],
                'accordion' => [
                    'label' => new lang_string('accordion', 'format_eloflexsections'),
                    'element_type' => 'advcheckbox',
                ],
                'cmbacklink' => [
                    'label' => new lang_string('cmbacklink', 'format_eloflexsections'),
                    'element_type' => 'advcheckbox',
                ],
            ];
    ...
    public function inplace_editable_render_section_name($section, $linkifneeded = true,
        ...
        if (empty($edithint)) {
            $edithint = new lang_string('editsectionname', 'format_eloflexsections');
        }
        if (empty($editlabel)) {
            $title = get_section_name($section->course, $section);
            $editlabel = new lang_string('newsectionname', 'format_eloflexsections', $title);
        }
        $section = $this->get_section($section);
        if ($linkifneeded && $section->collapsed != FORMAT_ELOFLEXSECTIONS_COLLAPSED) {
            $linkifneeded = false;
        }
    ...
    public function section_action($section, $action, $sr) {
        ...
        if ($section->section && ($action === 'setmarker' || $action === 'removemarker')) {
            // Format 'eloflexsections' allows to set and remove markers in addition to common section actions.
        ...
        if ($section->section && ($action === 'showexpanded' || $action === 'showcollapsed')) {
            ...
            $newvalue = ($action === 'showexpanded') ? FORMAT_ELOFLEXSECTIONS_EXPANDED : FORMAT_ELOFLEXSECTIONS_COLLAPSED;
    ...
    public function find_collapsed_parent($section, $returnid = false) {
        $section = $this->get_section($section);
        if (!$section->section || $section->collapsed == FORMAT_ELOFLEXSECTIONS_COLLAPSED) {
     ...
     *
     * format_eloflexsections processes additional attributes in the view course URL
    ...
    public function page_set_course(moodle_page $page) {
        ...
        if ($this->on_course_view_page()) {
            ...
            if ($switchcollapsed && confirm_sesskey() && has_capability('moodle/course:update', $context)
                ...
                if ($section->collapsed == FORMAT_ELOFLEXSECTIONS_EXPANDED) {
                    $newvalue = FORMAT_ELOFLEXSECTIONS_COLLAPSED;
                } else {
                    $newvalue = FORMAT_ELOFLEXSECTIONS_EXPANDED;
                }
                $this->update_section_format_options(array('id' => $section->id, 'collapsed' => $newvalue));
                if ($newvalue == FORMAT_ELOFLEXSECTIONS_COLLAPSED) {
                    if (!isset($options['sr'])) {
     ...
     *
     * Eloflexsections does not have a limit for the total number of the sections.
     *
     * @return int
     */
 ...
 */
function format_eloflexsections_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
    ...
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            ...
            [$itemid, 'eloflexsections'], MUST_EXIST);
 ...
 */
function format_eloflexsections_get_fontawesome_icon_map() {
    return [
        'format_eloflexsections:mergeup' => 'fa-level-up',
    ];
}

/**
 * If we are on an activity page inside the course in the 'eloflexsections' format - return the activity
 *
 * @return cm_info|null
 */
function format_eloflexsections_add_back_link_to_cm(): ?cm_info {
    global $PAGE, $CFG;
    if ($PAGE->course
            && $PAGE->cm
            && $PAGE->course->format === 'eloflexsections' // Only modules in 'eloflexsections' courses.
 ...
 * @return string
 */
function format_eloflexsections_before_footer() {
    global $OUTPUT;
    if ($cm = format_eloflexsections_add_back_link_to_cm()) {
        return $OUTPUT->render_from_template('format_eloflexsections/back_link_in_cms', [
            'backtosection' => [
