...
namespace format_eloflexsections;
 ...
 * @covers     \format_eloflexsections
 */
class format_eloflexsections_test extends \advanced_testcase {
    ...
    /**
     * Tests for format_eloflexsections::get_section_name method with default section names.
     */
    public function test_get_section_name() {
        ...
        $numsections = 5;
        $course = $generator->create_course(['numsections' => $numsections, 'format' => 'eloflexsections'],
            ['createsections' => true]);
    ...
    /**
     * Tests for format_eloflexsections::get_section_name method with modified section names.
     *
     * @return void
     */
    ...
    /**
     * Tests for format_eloflexsections::get_default_section_name.
     *
     * @return void
     */
    public function test_get_default_section_name() {
        ...
        foreach ($coursesections as $section) {
            if ($section->section == 0) {
                $sectionname = get_string('section0name', 'format_eloflexsections');
                $this->assertEquals($sectionname, $courseformat->get_default_section_name($section));
            } else {
                $sectionname = get_string('sectionname', 'format_eloflexsections') . ' ' . $section->section;
                $this->assertEquals($sectionname, $courseformat->get_default_section_name($section));
            }
        }
    }
    ...
    public function test_update_inplace_editable() {
        ...
        $this->setUser($user);
        $course = $this->getDataGenerator()->create_course(['numsections' => 5, 'format' => 'eloflexsections'],
            ['createsections' => true]);
        $section = $DB->get_record('course_sections', ['course' => $course->id, 'section' => 2]);

        // Call webservice without necessary permissions.
        try {
            core_external::update_inplace_editable('format_eloflexsections', 'sectionname', $section->id, 'New section name');
        ...
        $res = core_external::update_inplace_editable('format_eloflexsections', 'sectionname', $section->id, 'New section name');
    ...
    public function test_inplace_editable() {
        ...
        // Call callback format_eloflexsections_inplace_editable() directly.
        $tmpl = component_callback('format_eloflexsections', 'inplace_editable',
    ...
    public function test_default_course_enddate() {
        ...
        $params = ['format' => 'eloflexsections', 'numsections' => 5, 'startdate' => 1445644800];
    ...
    public function test_get_view_url() {
        ...
        $generator = $this->getDataGenerator();
        $course1 = $generator->create_course(['format' => 'eloflexsections']);
        course_create_sections_if_missing($course1, [0, 1]);

        $data = (object)['id' => $course1->id];
        /** @var \format_eloflexsections $format */
    ...
    /**
     * Tests for format_eloflexsections::delete_section_with_children.
     */
